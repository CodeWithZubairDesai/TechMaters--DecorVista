        ...
        if($request->ajax()) { 
            ...
            return Datatables::of($products)
                ...
                ->addColumn('price', function ($row) {
                    if ($row->has_variants == 0) {
                        // For fixed price, remove .00 if present
                        $price = rtrim(rtrim(number_format($row->fixed_price, 2), '0'), '.');
                        return 'Rs.'.$price;
                    } else {
                        // For variants, remove .00 from both price and weight, and return with <br> line breaks
                        $variantPrices = [];
                        foreach ($row->variants as $variant) {
                            // Remove .00 from price
                            $price = rtrim(rtrim(number_format($variant->price, 2), '0'), '.');
                            // Remove .00 from weight
                            $weight = rtrim(rtrim(number_format($variant->weight, 2), '0'), '.');
                            // Append weight and price together
                            $variantPrices[] = $weight . ' Grams : ' . 'Rs.'.$price;
                        }
                        // Join variant prices with line breaks
                        return implode('<br>', $variantPrices);
                    }
                })
